...
">
  <div style="display:flex; align-items:center; gap:0.7rem; margin-bottom:0.5rem;">
    ...
    ">{icon}</div>
    <div style="font-size:0.9rem; font-weight:700; color:{title_color}; line-height:1.25;">
      {title}
    </div>
  </div>
  <div style="font-size:0.78rem; color:#666; line-height:1.45;">{desc}</div>
  {progress_html}
  {unlocked_badge}
</div>
""",
...
if remaining > 0:
    ...
    if locked:
        ...
        st.markdown(
            ...
            f"{next_ach.title}</div>"
            f"<div style='font-size:0.82rem; color:#777;'>{next_ach.description}</div>"
            f"</div>"
            f"</div>",
            unsafe_allow_html=True,
        )
