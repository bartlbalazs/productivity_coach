...
ACHIEVEMENTS: list[Achievement] = [
    ...
    Achievement(
        ...
        icon="🔬",
        title="Laser Streak",
        description="Complete 3 consecutive sessions of 30+ minutes with zero distractions.",
        category="focus",
        condition=lambda s: s["max_zero_distraction_streak"] >= 3,
    ),
    Achievement(
        id="focus_factory",
        icon="🏭",
        title="Focus Factory",
        description="Complete 10 sessions of 45+ minutes with avg focus 8 or above.",
        category="focus",
        progress_target=10,
        progress_key="high_focus_session_count",
        condition=lambda s: s["high_focus_session_count"] >= 10,
    ),
    Achievement(
        id="relentless",
        icon="🛡️",
        ...
        category="focus",
        progress_target=25,
        progress_key="distraction_free_session_count",
        condition=lambda s: s["distraction_free_session_count"] >= 25,
    ),
    Achievement(
        id="ultra_marathon",
        icon="🦵",
        ...
        category="focus",
        condition=lambda s: any(ss["dur_min"] >= 240 for ss in s["sessions"]),
    ),
    # ── Consistency & Habits (new) ──────────────────────────────────────────
    Achievement(
        id="sunrise_regular",
        icon="🌄",
        title="Sunrise Regular",
        description="Complete 5 sessions that start before 7:00 AM.",
        category="consistency",
        progress_target=5,
        progress_key="early_bird_count",
        condition=lambda s: s["early_bird_count"] >= 5,
    ),
    Achievement(
        id="triple_header",
        icon="3️⃣",
        title="Triple Header",
        description="Complete 3 or more sessions in a single calendar day.",
        category="consistency",
        condition=lambda s: s["triple_header_days"] >= 1,
    ),
    Achievement(
        id="workweek_warrior",
        icon="📅",
        title="Workweek Warrior",
        description="Log at least one session on every day of the workweek (Mon–Fri) in the same week.",
        category="consistency",
        condition=lambda s: s["weekday_coverage_best_week"] >= 5,
    ),
    Achievement(
        id="weekend_ritual",
        icon="🗓️",
        title="Weekend Ritual",
        description="Complete sessions on both Saturday and Sunday across 4 different weekends.",
        category="consistency",
        progress_target=4,
        progress_key="weekend_pair_count",
        condition=lambda s: s["weekend_pair_count"] >= 4,
    ),
    # ── Health & Body (new) ─────────────────────────────────────────────────
    Achievement(
        id="ergonomic_elite",
        icon="🪑",
        title="Ergonomic Elite",
        description="Complete 10 sessions of 20+ minutes with zero posture corrections.",
        category="health",
        progress_target=10,
        progress_key="posture_free_session_count",
        condition=lambda s: s["posture_free_session_count"] >= 10,
    ),
    Achievement(
        id="recovery_beast",
        icon="🦁",
        title="Recovery Beast",
        description="Achieve 3 sessions with avg focus above 8, HRV above 60, and calm heart rate.",
        category="health",
        progress_target=3,
        progress_key="recovery_beast_sessions",
        condition=lambda s: s["recovery_beast_sessions"] >= 3,
    ),
    Achievement(
        id="calm_machine",
        icon="🧊",
        title="Calm Machine",
        description="Complete 5 sessions of 60+ minutes with an avg focus heart rate below 70 bpm.",
        category="health",
        progress_target=5,
        progress_key="calm_long_session_count",
        condition=lambda s: s["calm_long_session_count"] >= 5,
    ),
    Achievement(
        id="break_walker",
        icon="🚶",
        title="Break Walker",
        description="Log active recovery (1000+ steps during breaks) in 5 different sessions.",
        category="health",
        progress_target=5,
        progress_key="active_recovery_sessions",
        condition=lambda s: s["active_recovery_sessions"] >= 5,
    ),
    # ── Spotify & Audio (new) ───────────────────────────────────────────────
    Achievement(
        id="silent_streak",
        icon="🤫",
        title="Silent Streak",
        description="Complete 3 consecutive sessions of 30+ minutes with no music.",
        category="spotify",
        condition=lambda s: s["silent_streak"] >= 3,
    ),
    Achievement(
        id="soundtrack_addict",
        icon="🎶",
        title="Soundtrack Addict",
        description="Complete 25 sessions with music playing throughout.",
        category="spotify",
        progress_target=25,
        progress_key="music_session_count",
        condition=lambda s: s["music_session_count"] >= 25,
    ),
    Achievement(
        id="artist_explorer",
        icon="🗺️",
        title="Artist Explorer",
        description="Listen to 20 unique artists across all sessions.",
        category="spotify",
        progress_target=20,
        progress_key="global_unique_artist_count",
        condition=lambda s: s["global_unique_artist_count"] >= 20,
    ),
    Achievement(
        id="track_tourist",
        icon="🎫",
        title="Track Tourist",
        description="Play 50 unique tracks across all sessions.",
        category="spotify",
        progress_target=50,
        progress_key="global_unique_track_count",
        condition=lambda s: s["global_unique_track_count"] >= 50,
    ),
    # ── Secret / Funny (new) ────────────────────────────────────────────────
    Achievement(
        id="prompt_addict",
        icon="🤖",
        title="Prompt Addict",
        description="Trigger 10 or more AI coaching calls in a single session. The coach is basically your rubber duck at this point.",
        category="secret",
        secret=True,
        condition=lambda s: s["max_llm_calls_in_session"] >= 10,
    ),
    Achievement(
        id="journal_keeper",
        icon="📓",
        title="Journal Keeper",
        description="Add session notes in 10 different sessions. Journalling is productivity, apparently.",
        category="secret",
        secret=True,
        progress_target=10,
        progress_key="sessions_with_notes",
        condition=lambda s: s["sessions_with_notes"] >= 10,
    ),
    Achievement(
        id="weekend_zombie",
        icon="🧟",
        title="Weekend Zombie",
        description="Start a session between 3:00 AM and 5:00 AM on a weekend. Bold strategy.",
        category="secret",
        secret=True,
        condition=lambda s: s["has_weekend_zombie"],
    ),
    Achievement(
        id="mouse_marathon",
        icon="🖱️",
        title="Mouse Marathon",
        description="Move the mouse an absurd distance in a single session. Consider a wrist rest.",
        category="secret",
        secret=True,
        condition=lambda s: s["max_mouse_px_in_session"] >= 500_000,
    ),
    Achievement(
        id="click_goblin",
        icon="👆",
        title="Click Goblin",
        description="Log over 500 clicks in a single session. Are you debugging or playing Whac-A-Mole?",
        category="secret",
        secret=True,
        condition=lambda s: s["max_clicks_in_session"] >= 500,
    ),
    Achievement(
        id="monk_mode",
        icon="🧎",
        title="Monk Mode",
        description="Complete a 45+ min focus session with no music, no breaks, and almost no keyboard input. Either enlightened or AFK.",
        category="secret",
        secret=True,
        condition=lambda s: s["has_monk_mode"],
    ),
    Achievement(
        id="overexplainer",
        icon="📜",
        title="Overexplainer",
        description="Write a session goal longer than 30 words and also log notes during the session. The plan was longer than the work.",
        category="secret",
        secret=True,
        condition=lambda s: s["has_overexplainer"],
    ),
    Achievement(
        id="chaos_dj",
        icon="🌀",
        title="Chaos DJ",
        description="Skip through 15+ tracks in a session while producing below-average focus. The playlist wasn't the problem.",
        category="secret",
        secret=True,
        condition=lambda s: s["has_chaos_dj"],
    ),
    Achievement(
        id="rubber_ducker",
        icon="🦆",
        title="Rubber Ducker",
        description="Generate 5+ AI coaching prompts in a session. Sometimes you just need someone to talk at.",
        category="secret",
        secret=True,
        condition=lambda s: s["has_rubber_ducker"],
    ),
    Achievement(
        id="doom_scroller",
        icon="📉",
        title="Doom Scroller Energy",
        description="Log a 30+ min session with focus below 5 and suspiciously heavy mouse or click activity. We've all been there.",
        category="secret",
        secret=True,
        condition=lambda s: s["has_doom_scroller"],
    ),
]
